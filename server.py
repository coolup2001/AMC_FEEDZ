...
def main():
    parser = ArgumentParser(description="Tatti AMC Server")
    parser.add_argument('-a', '--host', help="Address to bind", dest='host', nargs=1, type=str, default=['0.0.0.0'])
    parser.add_argument('-p', '--port', help="Port to bind", dest='port', nargs=1, type=int, default=[8080])
    parser.add_argument('-v', '--version', action='version', version='%(prog)s 0.1')
    args = parser.parse_args()

    app.secret_key = os.urandom(12)
    app.run(port=args.port[0], host=args.host[0], debug=True)
